...
class KMeansCoreset(Coreset):
    ...
    def calc_sampling_distribution(self):
        ...
        centers = KMeans._init_centroids(self.X, self.n_clusters, self.init, random_state=self.random_state,x_squared_norms=x_squared_norms)
        sens = kmeans_sensitivity(self.X, self.w, centers, max(np.log(self.n_clusters), 1))
